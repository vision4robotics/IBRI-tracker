...
admm_gamma2 = params.admm_gamma2;
admm_gamma3 = params.admm_gamma3;
...
M_prev_1 = zeros(filter_sz_cell{1});
M_prev_2 = zeros(filter_sz_cell{1});
M_prev_3 = zeros(filter_sz_cell{1});
...
sr = construct_regwindow(sz, small_filter_sz{1});%ÕýÔòÏî
sr = 1 ./ sr;
...
%             scale_change_factor = scaleFactors(sind);
            ...
            shift_y_1 = id_ymax_curr - id_ymax_prev_1;
            shift_x_1 = id_xmax_curr - id_xmax_prev_1;
            sz_shift_y_1 = size(shift_y_1);
            sz_shift_x_1 = size(shift_x_1);
            if(sz_shift_y_1(1) > 1)
                shift_y_1 = shift_y_1(1);
            end
            if(sz_shift_x_1(1) > 1)
                shift_x_1 = shift_x_1(1);
            end
            M_prev_1= circshift(M_prev_1,shift_y_1,1);
            M_prev_1= circshift(M_prev_1,shift_x_1,2); 
            
            % do shifting of previous response map 2 
            shift_y_2 = id_ymax_curr - id_ymax_prev_2;
            shift_x_2 = id_xmax_curr - id_xmax_prev_2;
            sz_shift_y_2 = size(shift_y_2);
            sz_shift_x_2 = size(shift_x_2);
            if(sz_shift_y_2(1) > 1)
                shift_y_2 = shift_y_2(1);
            end
            if(sz_shift_x_2(1) > 1)
                shift_x_2 = shift_x_2(1);
            end

            M_prev_2 = circshift(M_prev_2,shift_y_2,1);
            M_prev_2 = circshift(M_prev_2,shift_x_2,2);
            
            % do shifting of previous response map 3   
            shift_y_3 = id_ymax_curr - id_ymax_prev_3;
            shift_x_3 = id_xmax_curr - id_xmax_prev_3;
            sz_shift_y_3 = size(shift_y_3);
            sz_shift_x_3 = size(shift_x_3);
            if(sz_shift_y_3(1) > 1)
                shift_y_3 = shift_y_3(1);
            end
            if(sz_shift_x_3(1) > 1)
                shift_x_3 = shift_x_3(1);
            end

            M_prev_3 = circshift(M_prev_3,shift_y_3,1);
            M_prev_3 = circshift(M_prev_3,shift_x_3,2);
    ...
    for k = 1: 1
            ...
            M1train=fft2(sr.*M_prev_1);
            M2train=fft2(sr.*M_prev_2);
            M3train=fft2(sr.*M_prev_3);
            % ADMM solving process
            while (i <= params.admm_iterations)
                A = mu / (admm_gamma + 1/T + admm_gamma2 + admm_gamma3);
                B = S_xx + A;
                S_lx = sum(conj(model_xf) .* l_f, 3);
                S_hx = sum(conj(model_xf) .* h_f, 3);
               %solve for Q
                q_f = (1 / mu) * ( ((1/T)*(bsxfun(@times, yf, model_xf)) + admm_gamma * bsxfun(@times, M1train, model_xf) + admm_gamma2 * bsxfun(@times, M2train, model_xf) + admm_gamma3 * bsxfun(@times, M3train, model_xf) - l_f + mu * h_f) - ...
                    bsxfun(@rdivide,((1/T)*(bsxfun(@times, model_xf, (S_xx .* yf))) + admm_gamma * bsxfun(@times, model_xf, (S_xx .* M1train)) + admm_gamma2 * bsxfun(@times, model_xf, (S_xx .* M2train)) + admm_gamma3 * bsxfun(@times, model_xf, (S_xx .* M3train)) -  bsxfun(@times, model_xf, S_lx) + mu * (bsxfun(@times, model_xf, S_hx))), B));

                %   solve for G
                g = (1/mu) * ifft2((mu*q_f) + l_f);
    ...
    if(seq.frame == 1)
            ...
            M_prev_1 = fftshift(response);
            max_M_prev_1 = max(M_prev_1(:));
            [id_ymax_prev_1,id_xmax_prev_1]= find(M_prev_1 == max_M_prev_1);
            id_ymax_prev_2 = id_ymax_prev_1;
            id_xmax_prev_2 = id_xmax_prev_1;
            id_ymax_prev_3 = id_ymax_prev_2;
            id_xmax_prev_3 = id_xmax_prev_2;
            ...
            id_ymax_prev_2 = id_ymax_prev_1;
            id_xmax_prev_2 = id_xmax_prev_1;
            id_ymax_prev_1 = id_ymax_curr;
            id_xmax_prev_1 = id_xmax_curr;
